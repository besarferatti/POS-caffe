...
const esc = (x: unknown) => typeof x === "number" ? String(x) : `"${String(x ?? "").replaceAll('"', '""')}"`;
...
export async function GET(request: NextRequest) {
  ...
  const rowsWithCurrency = rows.map((row: Record<string, unknown>) => ({ ...row, currency_code: currencyCode }));
  const keys = [...(kind === "orders" ? orderColumns : rows.length ? Object.keys(rows[0]) : []), "currency_code"];
  const csv = [keys.map(esc).join(","), ...rowsWithCurrency.map((r: Record<string, unknown>) => keys.map(k => esc(r[k])).join(","))].join("\n");
