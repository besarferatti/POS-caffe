...
export default async function ReportsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const startAt = `${start}T00:00:00.000Z`; const endAt = new Date(`${end}T00:00:00.000Z`); endAt.setUTCDate(endAt.getUTCDate()+1);
  const { data, error } = await supabase.rpc("reporting_dashboard", { range_start: startAt, range_end: endAt.toISOString(), staff_id: get("staff") || null, method_id: get("method") || null, category_id: get("category") || null });
