...
export default async function OrdersPage() {
  ...
  const [{ data: floorLayout, error: floorLayoutError }, { data: categories, error: categoriesError }, { data: products, error: productsError }, { data: openOrders, error: openOrdersError }] = await Promise.all([
  ...
  const tables = floorLayout ? tablesFromFloorLayout(floorLayout.layout).map((table) => ({ ...table, status: (openTableIds.has(table.id) ? "occupied" : table.status) as TableStatus })) : [];
