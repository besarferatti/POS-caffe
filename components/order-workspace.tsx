  ...
  async function completePayment(internalOnly = false) {
    ...
    if (internalOnly) {
      const { data, error } = await createClient().rpc("complete_order_internal_only", {
        ...
        job_payload: { table: selectedTable?.name, order_id: activeOrder.id, items: items.map((item) => ({ name: item.product?.[0]?.name, quantity: item.quantity, notes: item.notes })) },
