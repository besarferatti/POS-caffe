...
type Table = { id: string; name: string; status: "available" | "occupied" | "reserved"; x?: number; y?: number; width?: number; height?: number; rotation?: number; shape?: "round" | "square" | "rectangle" };
type FloorObject = { id: string; type: string; label: string; x: number; y: number; width: number; height: number; rotation: number; shape?: "round" | "square" | "rectangle"; isTable: boolean };
...
export function OrderWorkspace({ tables, objects, floorError, categories, products, openOrders }: { tables: Table[]; objects: FloorObject[]; floorError?: string; categories: Category[]; products: Product[]; openOrders: Order[] }) {
