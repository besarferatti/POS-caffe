...
export function OrderWorkspace({ tables: initialTables, objects, floorError, catalogError, categories, products }: { tables: Table[]; objects: FloorObject[]; floorError?: string; catalogError?: string; categories: Category[]; products: Product[]; openOrders: Order[]; openOrdersError?: string }) {
  const [tables, setTables] = useState(initialTables); const [selectedTable, setSelectedTable] = useState<Table | null>(null); const [activeOrder, setActiveOrder] = useState<Order | null>(null); const [items, setItems] = useState<OrderItem[]>([]); const [categoryId, setCategoryId] = useState(categories[0]?.id ?? ""); const [busy, setBusy] = useState(false); const [message, setMessage] = useState(""); const [orderNotes, setOrderNotes] = useState(""); const [paymentOpen, setPaymentOpen] = useState(false); const [paymentMethods, setPaymentMethods] = useState<PaymentMethod[]>([]); const [paymentMethodId, setPaymentMethodId] = useState(""); const [paymentAmount, setPaymentAmount] = useState(""); const [tendered, setTendered] = useState(""); const [internalReason, setInternalReason] = useState("");
  ...
  async function sendInternal() {
    ...
    const { data, error } = await createClient().rpc("queue_internal_print_job", {
      ...
      job_payload: { table: selectedTable?.name, order_id: activeOrder.id, currency_code: currencyCode, items: additions.map((item) => ({ name: item.product?.[0]?.name, quantity: item.quantity - item.sent_quantity, notes: item.notes })) },
  ...
  async function completePayment(internalOnly = false) {
    ...
    if (internalOnly && !internalReason.trim()) { setMessage("Please provide a reason before completing this order as internal-only."); return; }
    setBusy(true);
    if (internalOnly) {
      const { data, error } = await createClient().rpc("complete_order_internal_only", {
        target_order_id: activeOrder.id,
        reason: internalReason,
        job_payload: { table: selectedTable?.name, order_id: activeOrder.id, currency_code: currencyCode, items: items.map((item) => ({ name: item.product?.[0]?.name, quantity: item.quantity, notes: item.notes })) },
        job_items: items.map((item) => ({ order_item_id: item.id, quantity: item.quantity }))
      }).single();
      if (error || !data) setMessage(friendlyError(error));
      else releaseOrder("Internal-only order completed, internal ticket queued, and table released.");
      setBusy(false);
      return;
    }
