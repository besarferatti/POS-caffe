-- Keep reports independent from the legacy normalized dining_tables representation.
-- Floor-plan table objects are the authoritative source when no legacy table exists.
drop function if exists public.reporting_dashboard(timestamptz, timestamptz, uuid, uuid, uuid);

create function public.reporting_dashboard(
...
begin
  ...
  with scoped as (
    select
      o.*,
      coalesce(dt.name, floor_table.table_label, 'Table ' || left(o.table_id::text, 8)) as table_label,
      coalesce(p.display_name, 'Unassigned') as worker
    from public.orders o
    left join public.dining_tables dt on dt.id = o.table_id
    left join public.floor_layouts fl on fl.id = 1
    left join lateral (
      select coalesce(table_object.value->>'label', table_object.value->>'name', table_object.value->>'number') as table_label
      from jsonb_path_query(
        coalesce(fl.layout, '{}'::jsonb),
        ...
        jsonb_build_object('table_id', to_jsonb(o.table_id::text))
      ) as table_object(value)
      where jsonb_typeof(table_object.value) = 'object'
        and coalesce(table_object.value->>'label', table_object.value->>'name', table_object.value->>'number') is not null
      limit 1
    ) floor_table on true
    left join public.profiles p on p.id = coalesce(o.assigned_worker, o.opened_by, o.created_by)
    where o.opened_at >= p_range_start
      and o.opened_at < p_range_end + interval '1 day'
      and (p_staff_id is null or coalesce(o.assigned_worker, o.opened_by, o.created_by) = p_staff_id)
      and (p_category_id is null or exists (
        select 1 from public.order_items oi
        join public.products pr on pr.id = oi.product_id
        where oi.order_id = o.id and pr.category_id = p_category_id
      ))
  ), paid_payments as (
    select py.*, pm.name method_name, pm.code method_code
    from public.payments py
    join public.payment_methods pm on pm.id = py.payment_method_id
    join scoped o on o.id = py.order_id
    where p_method_id is null or py.payment_method_id = p_method_id
  ), paid_orders as (
    select * from scoped where status::text in ('paid', 'closed') and not internal_only
  ), product_sales as (
    select oi.product_name_snapshot name, sum(oi.quantity) quantity,
      sum(oi.quantity * oi.unit_price_snapshot - oi.line_discount) sales
    from public.order_items oi
    join paid_orders o on o.id = oi.order_id
    join public.products pr on pr.id = oi.product_id
    where oi.deleted_at is null and oi.status = 'active'
