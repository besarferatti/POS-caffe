...
alter table public.categories
  ...
  add column if not exists printer_destination text not null default 'internal'
    check (printer_destination in ('internal', 'none')),
...
create index if not exists categories_sort_active_idx
...
alter table public.orders
  drop constraint if exists orders_closed_at_matches_status;

alter table public.orders
  add constraint orders_closed_at_matches_status
  check (
    (status::text in ('paid', 'closed', 'cancelled', 'voided'))
    =
    (closed_at is not null)
  );

-- Keep exactly one active order per floor-table UUID.
...
create unique index orders_one_active_order_per_table
  ...
  where status in (
    ...
    'sent'::public.order_status,
    'partially_paid'::public.order_status,
    'internal_only'::public.order_status
  );
...
alter table public.order_items
  ...
  add column if not exists printed_quantity integer not null default 0
    check (printed_quantity >= 0),
...
set
  ...
  unit_price_snapshot = oi.price,
  tax_rate_snapshot = p.tax_rate
...
create unique index if not exists one_active_reservation_per_floor_table
  on public.table_reservations (table_object_id)
...
create table if not exists public.payment_methods (
  ...
  requires_fiscal boolean not null default true,
...
create table if not exists public.printers (
  ...
  type text not null check (type in ('internal', 'fiscal')),
  enabled boolean not null default true,
  connection_method text not null default 'local_agent',
  ip_address inet,
...
create table if not exists public.print_attempts (
  ...
  status text not null check (status in ('processing', 'printed', 'failed')),
...
create policy "catalog read with orders permission"
  on public.categories
  for select
  to authenticated
  using (public.has_permission('orders:view'));

create policy "catalog manage with permission"
  on public.categories
  for all
  to authenticated
...
create policy "products read with orders permission"
  on public.products
  for select
  to authenticated
  using (public.has_permission('orders:view'));

create policy "products manage with permission"
  on public.products
  for all
  to authenticated
  using (public.has_permission('products:manage'))
...
create policy "reservations use with table permission"
  on public.table_reservations
  for all
  to authenticated
  using (public.has_permission('tables:reserve'))
  with check (public.has_permission('tables:reserve'));

create policy "reservations read with orders permission"
  on public.table_reservations
  for select
  to authenticated
  using (public.has_permission('orders:view'));

create policy "payment methods read for payments"
  on public.payment_methods
  for select
  to authenticated
  using (public.has_permission('orders:pay'));

create policy "payments use with payment permission"
  on public.payments
  for all
  to authenticated
  using (public.has_permission('orders:pay'))
  with check (public.has_permission('orders:pay'));

create policy "printers manage with permission"
  on public.printers
  for all
  to authenticated
  using (public.has_permission('printers:manage'))
  with check (public.has_permission('printers:manage'));

create policy "print jobs manage with permission"
...
create policy "print job items manage with permission"
...
create policy "print attempts manage with permission"
  on public.print_attempts
  for all
  to authenticated
  using (public.has_permission('printers:manage'))
  with check (public.has_permission('printers:manage'));
...
create policy "orders read with permission"
  on public.orders
  for select
  to authenticated
  using (public.has_permission('orders:view'));

create policy "orders create with permission"
  on public.orders
  for insert
  to authenticated
  with check (
...
create policy "orders edit with permission"
  on public.orders
  for update
  to authenticated
...
create policy "items read with order permission"
  on public.order_items
  for select
  to authenticated
  using (public.has_permission('orders:view'));

create policy "items create with order permission"
  on public.order_items
  for insert
  to authenticated
  with check (public.has_permission('orders:edit'));

create policy "items edit with order permission"
  on public.order_items
  for update
  to authenticated
  using (public.has_permission('orders:edit'))
...
values
  ...
  ('printers:manage', 'Manage and retry print jobs'),
  ('floor_plan:edit', 'Edit floor plan'),
  ('tables:reserve', 'Reserve tables'),
  ('tables:transfer', 'Transfer or merge tables'),
...
values
  ...
  ('manager', 'tables:reserve'),
  ('manager', 'tables:transfer'),
  ('worker', 'orders:view'),
...
comment on table public.print_jobs is
  'Queue consumed by an authenticated local print agent. Payload contains ticket data and idempotency_key; agent acknowledges printed or failed and records a print_attempt.';
...
begin
  if auth.uid() is null
     or not public.has_permission('orders:create') then
    raise exception 'Not authorized to open orders';
  end if;

  perform 1
  from public.floor_layouts
  where id = 1
    and jsonb_path_exists(
      ...
      '$.** ? (
        ...
        &&
        (
          @.type like_regex ".*table.*" flag "i" ||
          @.objectType like_regex ".*table.*" flag "i" ||
          @.kind like_regex ".*table.*" flag "i" ||
          @.tableType like_regex ".*table.*" flag "i" ||
          @.tableShape like_regex "^(round|square|rectangle|rectangular)$" flag "i" ||
          @.shape like_regex "^(round|square|rectangle|rectangular)(-table)?$" flag "i"
        )
      )',
      jsonb_build_object(
        'table_id',
        to_jsonb(target_table_id::text)
      )
    )
  for update;

  if not found then
    raise exception 'Floor table not found';
  end if;
...
grant execute
on function public.open_order_for_table(uuid)
to authenticated;
