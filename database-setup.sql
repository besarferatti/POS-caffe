...
create unique index if not exists orders_one_active_order_per_table on public.orders(table_id) where status::text in ('open', 'sent', 'partially_paid');
...
begin
  ...
  perform 1 from public.floor_layouts where id = 1
    ...
    for update;
  if not found then raise exception 'Floor table not found'; end if;
  select * into result from public.orders where table_id = target_table_id and status::text in ('open', 'sent', 'partially_paid') for update;
  if found then return result; end if;
...
-- Phase 5: completed internal-only orders.
